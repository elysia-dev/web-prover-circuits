...
export function nearestMultiplePad(
  input: number[],
  multiple: number
): number[] {
...
export function PolynomialDigest(
  coeffs: number[],
  input: bigint,
  counter: bigint
): bigint {
...
export function modAdd(...args: bigint[]): bigint {
  return args.reduce(
    (acc, val) => (((acc + val) % PRIME) + PRIME) % PRIME,
    BigInt(0)
  );
}
...
export function InitialDigest(
  ...
  const startLineDigest = PolynomialDigest(
    startLineBytes,
    ciphertextDigest,
    BigInt(0)
  );
  ...
  const jsonSequenceDigestHash = poseidon1([
    compressTreeHash(ciphertextDigest, jsonTreeHash),
  ]);
...
export function CombinedInitialDigest(
  ...
  for (var i = 0; i < request_ciphertexts.length; i++) {
    ciphertextDigests.push(
      DataHasher(
        request_ciphertexts[i],
        ciphertextDigests[ciphertextDigests.length - 1]
      )
    );
  }
  ...
  const requestStartLineDigest = PolynomialDigest(
    requestStartLineBytes,
    ciphertextDigest,
    BigInt(0)
  );
  ...
  const responseStartLineDigest = PolynomialDigest(
    responseStartLineBytes,
    ciphertextDigest,
    BigInt(0)
  );
  ...
  let allDigests = [
    requestStartLineDigest,
    responseStartLineDigest,
    ...requestHeadersDigest,
    ...responseHeadersDigest,
  ];
  ...
  let initialHttpMachineStateDigest = PolynomialDigest(
    initialHttpMachineState,
    ciphertextDigest,
    BigInt(0)
  );
